import { buildingDistance } from './data'
...
function evaluate(picked:Course[],all:Course[],enabled:Set<GlobalCondition>):ScheduleResult{
 ...
 days.forEach(d=>{const ms=grouped.get(d)!.sort((a,b)=>a.start-b.start);if(!ms.length)return;active++;if(!ms.some(m=>m.start<13&&m.end>12))lunch++;ms.forEach((m,i)=>{if(m.start<10)morning++;if(i){const prev=ms[i-1],gap=Math.max(0,Math.round((m.start-prev.end)*60));if(gap>10)gaps+=gap;if(gap<=10){const minutes=buildingDistance[prev.building]?.[m.building]??10;moves+=minutes;const from=picked.find(c=>c.meetings.includes(prev)),to=picked.find(c=>c.meetings.includes(m));moveDetails.push({day:d,fromCourse:from?.name??'',toCourse:to?.name??'',fromBuilding:prev.building,toBuilding:m.building,minutes})}}})})
